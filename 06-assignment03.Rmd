...
Please read through the Multi-Item Measures section before working on this assignment.
...
1. Load the `tidyverse` and `psych` packages using `library()`.
1. Read in the data file `WVS2012 Singapore BFI10 Data.csv` using `read_csv()` or `read.csv()`. Name this data frame `data`.
1. Examine `data` with `glimpse()` or `View()` or `str()`. Check that you have 1972 rows and 10 columns. 
1. Create a subset of the data by removing all participants who had negative values for ANY of the BFI10 items (e.g., -5, -2). Name this subset `subset`. (Hint: Use the `filter_all()` and `all_vars()` functions from the `dplyr` package. Or, use the `filter()` in combination with the `if_all()` functions. You'll need to read up a bit on the internet to figure out how they work and then experiment.) This is probably the hardest part of this assignment. But if all goes well, you should have 1963 rows (participants) in your subset.

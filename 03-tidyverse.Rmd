...
Before doing that, though we need to set the working directory. The reason we need to set the working directory is that we need to tell R where to find the .csv file that we saved. To do that, in RStudio, go to **Session > Set Working Directory > Choose Directory**. Select the folder where you saved the RData file. 

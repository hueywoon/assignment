...
The code you used to run the one-sample t test can be expressed differently. This is known as the formula interface version: 

```{r eval = FALSE}

# the difference is that instead of specifying x = swls2019_avg...
# it is expressed as swls2019_avg ~ 1 
# This follows the formula: DV ~ IV. Because there isn't an IV in a one-sample t test, the right hand side is coded as 1. 
...
# Notice that we enter the arguments the same way as we would in the formula interface version of t.test()
...
# Conduct the correlated groups t test to compare the satisfaction with life scores in 2019  and in 2021 to see if there is any significant change over the two years. 
# x = data$swls2019_avg tells R what the first set of scores is
# y = data$swls2021_avg tells R what the second set of scores is
# alternative = c("two.sided") tells R we want to conduct a two-tailed test
# mu = 0 tells R that we're comparing the difference between the 2019 and 2021 scores against the value of 0
# paired = TRUE tells R that the observations are paired, meaning each individual offered pairs of observations (i.e., swls2019_avg and swls2021_avg)
# conf.level = 0.95 tells R we want the 95% confidence level 
...
Similar to the one-sample t test, the correlated groups t test also has a formula interface version: 

```{r eval = FALSE}

t.test(Pair(swls2019_avg, swls2021_avg) ~ 1, data = data, alternative = c("two.sided"), mu = 0, conf.level = 0.95)

# Notice that the paired = TRUE argument has been taken out because the same information has been replaced by the Pair(swls2019_avg, swls2021_avg) argument
# If you leave the paired = TRUE argument in, R will throw up an error

```

Check that the formula interface gives you the same result as before. 
...
# Again, notice that we enter the arguments the same way as we would in the formula interface version of t.test()
...
The descriptive statistics tell us that on average, the males have lower satisfaction with life scores than females. To find out whether this result is statistically significant, we need to conduct the independent groups t test. 

```{r}

# We conduct the independent groups t test
# Notice that this t.test code takes the form of the formula interface: DV ~ IV
# alternative = c("two.sided") tells R we want to conduct a two-tailed test
# mu = 0 tells R we're comparing the difference between males and females against the value of 0
...
_Note._ Again, note that there is a negative sign here because R took `male - female`. If R took `female - male`, Cohen's d will be a positive number. 
...
mod1 <- aov(swls2019_avg ~ marital_status, data = data)  
...
eta_squared(mod1, ci = 0.95, alternative = "two.sided")
...
describeBy(data$swls2019_avg, group = data$gender)
describeBy(data$swls2019_avg, group = data$have_children)
describeBy(data$swls2019_avg, group = list(data$gender, data$have_children))
...
mod2 <- aov(swls2019_avg ~ gender + have_children + gender*have_children, data = data) 
...
We can get the effect size, eta-squared (η2), for each of the main effects as well as the interaction effect using the `eta_squared()` function from `effectsize` package. 

```{r}

eta_squared(mod2, ci = 0.95, alternative = "two.sided")
...
# Unlike cor(), cor.test() gives you the significance level
...
Both ways would give us the same result as the earlier one. 
...
t.test(x = x, y = y, alternative = "two.sided", mu = 0, paired = TRUE, conf.level = 0.95)
...
mod3 <- lm(data$swls2021_avg ~ data$swls2019_avg)
summary(mod3)
...
Congratulations!!! You've come to the end of the hypothesis testing. 

With this, you will be able to complete the remaining assignments!
...
To complete the take home assignments, you need both R and RStudio. These are  _different_ programmes. I think of R as the software that executes your commands and RStudio as the interface between us and R that makes interacting with R a much more pleasant experience. For example, RStudio has keyboard shortcuts that allow you to paste specific code with just a few keystrokes so you don't have to type them over and over. This makse writing R code just a tad faster!
...
1.	Under **Download and Install R**, Click on **Download R for Windows**. Click on *base*.
...
To open a blank R script, go to _File > New File > R Script_. Or, you can use the shortcut Ctrl + Shift + N (Windows) or 	Cmd + Shift + N (macOS). Notice now, your RStudio has four panes. The script editor now takes up the top half of the left hand side of the screen. 
...
1.	To run (execute) a line in the script file, place the cursor on the line, hit Ctrl +  Enter (Windows) or Cmd + Enter (macOS).
...
1.	To clear the console when it becomes too messy, hit Ctrl + L (Windows) or Cmd + L (macOS).
...
x <- 9  # what this means is that x gets the value of 9

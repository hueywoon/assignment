...
To analyse data in R, you need both R and RStudio. These are _different_ programmes. Think of R as the software that executes our commands and RStudio as the interface between us and R that makes interacting with R a much more pleasant experience. RStudio has many nifty features, like providing code suggestions and the keyboard shortcuts that make writing code much faster and easier. For the sake of your sanity, I strongly recommend you download both programmes!
...
I strongly believe that people should be able to reproduce each others' analyses. This provides the checks and balances that is important for science to advance. Therefore, you should be willing and able to share your code. But if your code is unintelligible, it will be entirely useless. Therefore, there are certain conventions that I recommend you follow when you write R code so that the code is understandable to others. These conventions are laid out in the tidyverse Style Guide. You can read the Style Guide [HERE](https://style.tidyverse.org/syntax.html). 

### Using R Projects

When we analyse data, we may have a bunch of different files from different projects all strewn in a single directory (folder). This can get quite messy. It is especially confusing if you have relative paths. To stay organised, it is best that we create a project.
...
If you choose not to use R projects, 
